...
const TargetCursor = ({
  ...
  // Check if true mobile user agent (phone/tablet) to prevent disabling on touch-screen laptops
  ...
  useEffect(() => {
    ...
    // Use direct Lerp updating instead of recursive gsap.to inside the ticker loop.
    // This solves the overlapping/canceling tween bug that makes corners cluster in the center.
    const tickerFn = () => {
      ...
      corners.forEach((corner, i) => {
        ...
        const easeFactor = strength === 1 ? (parallaxOn ? 0.25 : 1) : strength;
        const finalX = currentX + (targetX - currentX) * easeFactor;
        const finalY = currentY + (targetY - currentY) * easeFactor;
